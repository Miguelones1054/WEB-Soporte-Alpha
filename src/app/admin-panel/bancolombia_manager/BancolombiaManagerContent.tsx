...
export interface BancolombiaManagerContentProps {
  embedded?: boolean;
  adminInfo?: AdminInfo | null;
  onBackToHub?: () => void;
}

export function BancolombiaManagerContent({
  embedded = false,
...
✅ Operación completada exitosamente`;
  ...
  const sendUserNotification = async () => {
    ...
    if (!usuarioKey) {
      openResultModal('No se pudo identificar el usuario Bancolombia. Vuelva a buscar.', 'error');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const raw = parseApiErrorDetail(result.detail, 'Error al enviar notificación');
  ...
  return (
    <div className={embedded ? 'retro-app-manager' : 'min-h-screen bg-gray-900'}>
      ...
      <main className={embedded ? 'retro-app-manager__main' : 'flex-1 p-6'}>
        <AppGlobalNotificationControl app="bancolombia" embedded={embedded} />
                      ...
                      <input
                        ...
                        className={
                          embedded
                            ...
                            : 'w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500 transition-colors'
                        }
                        placeholder={embedded ? 'Login del usuario' : 'Usuario'}
                      />
                    </div>
                    <button
                      type="submit"
                      ...
                      className={
                        embedded
                          ...
                          : 'w-full border border-amber-500/90 bg-transparent text-amber-200 py-3 px-6 rounded-md transition-all duration-200 font-semibold disabled:cursor-not-allowed disabled:opacity-50 disabled:border-gray-600 disabled:text-gray-500 enabled:hover:border-amber-400 enabled:hover:text-amber-100'
                      }
                    >
              ...
              <div className="retro-user-panel__card">
                <div className="retro-user-panel__header">
                  <div className="retro-user-panel__avatar">
                    <img src="/bancolombia-logo.png" alt="Bancolombia" className="object-contain" />
                  </div>
                  <div className="retro-user-panel__identity">
                    <h4>{userData.username || userData.usuarioLogin}</h4>
                    <p>
                      <RetroIcon name="office/document" size={14} alt="" />
                      Número de cuenta:{' '}
                      <span className="font-mono">{userData.numeroCuenta || '—'}</span>
                    </p>
                    <p className="retro-user-panel__phone">
                      <RetroIcon name="users/user_card" size={16} alt="" />
                      <span className="retro-user-panel__phone-label">Usuario Login:</span>
                      <span className="retro-user-panel__phone-value">{userData.usuarioLogin}</span>
                    </p>
                  </div>
